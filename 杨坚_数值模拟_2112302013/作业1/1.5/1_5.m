...
filename = '迎风格式_test.gif'; % 定义将要创建的动画文件的名称

count=0;plt = 50;
while count<10000
    count=count+1;
    % 迎风格式(u>0)
    T2(xi)= T1(xi)- lambda*( T1(xi)-T1(xi-1)  );
    
    % %boundary condition
    T2(1)= T1(1)- lambda*( T1(2)-T1(end)  );
    T2(end)= T1(end)- lambda*( T1(1)-T1(end-1)  );
...
axis tight manual % 设置坐标轴
% 设置坐标轴属性，使得坐标轴的边界紧贴图形，并且手动设置，不自动调整。
filename = 'CTCS_test.gif'; % 定义将要创建的动画文件的名称
